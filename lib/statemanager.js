...
StateManager.prototype.queueTransaction = function(method, txJsonRpc, blockNumber, callback) {
  ...
  if (method === "eth_call") {
    this.processCall(from, tx, blockNumber, callback);
  } else {
...
StateManager.prototype.getLogs = function(filter, callback) {
  ...
  async.parallel(
    ...
    function(err, results) {
      ...
      // If we're a fork, and some of this request is from before the fork, then make an API request for those logs
      if (self.options.fork){
        fromBlock = self.blockchain.forkBlockNumber + 1;
      }
      ...
      async.parallel(
        {
          // Note that while we define localLogs and remoteLogs here, each function is actually
          // affecting the logs array - we are not using return values.
          localLogs: function(done) {
            return async.whilst(
              function() {
          ...
          remoteLogs: function(done){
            self.blockchain.web3.currentProvider.send(
              {
                jsonrpc: "2.0",
                method: "eth_getLogs",
                params: [
                  {
                    fromBlock: to.hex(results.fromBlock),
                    toBlock: to.hex(fromBlock),
              ...
              function(err, res) {
                ...
                var remoteLogs = res.result.map(function(log) {
                  // To make this result masquerade as the right information.
                  log.block = {hash: () => log.blockHash, header: {number: log.blockNumber} };
                  return new Log(log);
                });

                logs.push.apply(logs, remoteLogs);
                done();
              }
            );
          }
