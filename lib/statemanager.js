...
StateManager.prototype.processCall = function(from, tx, blockNumber, callback) {
  ...
  self.createTransactionWithCorrectNonce(tx, from, blockNumber, function(err, tx) {
    if (err) {
      return callback(err);
    }
...
StateManager.prototype.processTransaction = function(from, tx, callback) {
  ...
  self.createTransactionWithCorrectNonce(tx, from, 'latest', function(err, tx) {
    if (err) {
      return callback(err);
    }
...
StateManager.prototype.createTransactionWithCorrectNonce = function(tx, from, blockNumber, callback) {
