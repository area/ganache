...
ForkedBlockchain.prototype.initialize = function(accounts, callback) {
  ...
  this.memoizedGetCode = memoize(this.web3.eth.getCode, { primitive: true, async: true, length: 3 });
  this.memoizedGetBalance = memoize(this.web3.eth.getBalance, { primitive: true, async: true, length: 3 });
  this.memoizedGetTransactionCount = memoize(this.web3.eth.getTransactionCount, {
    primitive: true,
    async: true,
    length: 3
  });
...
ForkedBlockchain.prototype.fetchCodeFromFallback = function(address, blockNumber, callback) {
  ...
  this.getSafeFallbackBlockNumber(blockNumber, function(err, safeBlockNumber) {
    ...
    self.memoizedGetCode(address, safeBlockNumber, function(err, code) {
      if (err) {
        return callback(err);
      }
...
ForkedBlockchain.prototype.fetchBalanceFromFallback = function(address, blockNumber, callback) {
  ...
  this.getSafeFallbackBlockNumber(blockNumber, function(err, safeBlockNumber) {
    ...
    self.memoizedGetBalance(address, safeBlockNumber, function(err, balance) {
      if (err) {
        return callback(err);
      }
...
ForkedBlockchain.prototype.fetchNonceFromFallback = function(address, blockNumber, callback) {
  ...
  this.getSafeFallbackBlockNumber(blockNumber, function(err, safeBlockNumber) {
    ...
    self.memoizedGetTransactionCount(address, safeBlockNumber, function(err, nonce) {
      if (err) {
        return callback(err);
      }
